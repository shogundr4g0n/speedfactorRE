...
//List of actions with their relative bonuses
    ...
    if(!game.user.isGM) {
        ...
        let combatants = game.combat.combatants.filter(c => !c.actor.isPC);
        for(let combatant of combatants) {
            let combActor = game.actors.get(combatant.actor.id)
            let combToken = canvas.tokens.get(combatant.tokenId)
            let data = [
            ...
            let rv = await quick_dialog({data}, combActor.name);
            rollInit(combToken, rv)
        }
    }
